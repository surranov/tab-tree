...
- Package hygiene: `.idea/` (IntelliJ metadata) and `.DS_Store` are now excluded from the published `.vsix` via `.vscodeignore`.
...
- Redundant intro paragraph from README — the tagline, screenshot, and "Why" section already cover the same ground.

## [0.0.5] - 2026-04-16

### Fixed

- README badges now load — switched from `img.shields.io/visual-studio-marketplace/*` (retired upstream) to `vsmarketplacebadges.dev`.
